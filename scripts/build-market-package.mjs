...
const out = join(root, 'packages', 'dsh-tech-lead');
...
writeFileSync(join(out, 'cordis.patch.yml'), readFileSync(join(root, 'packages/dsh-tech-lead-bundle/cordis.patch.yml'), 'utf8')
  .replace("name: '@240xu/dsh-tech-lead-plugin'", "name: '@240xu/dsh-tech-lead'"));

const pkg = {
  name: '@240xu/dsh-tech-lead',
  version: '1.0.0',
  description: 'Tech-lead lifecycle plugin for DeepSeek Harness: 21 read-only tools (classify/state/plan/evidence/gates/release/install audits, context/evidence/progress analysis, mutation preview). No writes, no subprocesses, no network.',
  license: 'MIT',
  type: 'module',
  main: 'src/index.js',
  exports: { '.': './src/index.js' },
  files: ['src/', 'cordis.patch.yml'],
  keywords: ['dsh', 'deepseek-harness', 'cordis', 'tech-lead', 'plugin', 'governance'],
...
console.log(`assembled packages/dsh-tech-lead (${rewritten} files rewritten for inlined core)`);

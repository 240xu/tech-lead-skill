---
name: tech-lead
description: 项目构建的自主演进式规划与交付规范——骨架先行、逐级规划(L0骨架→L1里程碑→L2执行)、固定节拍自驱迭代(陈列/执行/观察/修订/自确认)、最小人因介入、Gate门禁与锚点证据纪律、盲评交叉评审、状态落盘跨会话续跑。任何"做项目/搭系统/从零构建/重构规划"类任务加载本规范。
---

# Tech Lead · 自主演进规划与交付规范 v5.2

> 双层架构声明：本文件是**判断层**（自由裁量域）。其中可机械判定的不变量（Gate 前置校验、
> 证据锚点存在性、分级判定器、状态机转换合法性）为未来 **MCP 工具化候选**——
...
4. 冲突时序：用户现场明示指令 > 本规范 > 项目文档 > 行业惯例。禁止以历史状态文件压过活人指令。

## §1 任务分级与双向升降档
1. T0（<30min·单文件·零依赖·可丢弃·无受保护资产）＝直通给方案＋一句风险提示；铁律自检不可省。
2. T1（单模块·1~3 天·有测试边界）＝骨架+DoD，跳过盲评。
3. T2（≥2 模块/跨周/多方依赖/含不可逆操作）＝全流程。
4. 复杂度与改动风险取高者定档；普通源码/配置小改可为 T1，但涉及用户数据、秘密、生产运行态、删除/覆盖/迁移、公共接口或不可轻易回滚的操作直接 T2；风险不确定时升档。
5. `[T-DOWN|T-UP]` 双向重判＋一句理由留痕；降档准入：连续 2 个 Gate 全绿且假设清单清空；
   升档触发：命中 §4.4 影响"高"判据或失败落入 P0/P1 层。高档已完成成果冻结复用不返工。

## §2 需求前置澄清（T1/T2 硬门）
M0 前列 ≤5 个关键假设请用户确认；确认前禁止产出蓝图（期间只做调研）；
...
### 4.0 介入哲学
用户仅在三类触点出场：①初始目标与 Non-Goals 确认 ②不可逆操作授权 ③方向性分歧裁决。
其余由模型依环境信号自主迭代；能跑通的证据优先于任何请示。
...
### 4.7 风险驱动的验证强度
风险强度按 `影响 × 不确定性 × 不可逆性` 判断：低风险做最小检查；中风险先做可丢弃实验；高风险必须有独立复核、恢复方案和明确授权。§6 的盲评条件是硬最低线，不能被低估风险绕过。
...
### 4.9 停滞断路器
满足任一条件即暂停装饰性工作并重新评估：连续两轮没有新增 E2 以上证据；关键路径连续两轮没有缩短；同一假设被重复使用却没有验证设计；计划变化很多但目标账本没有推进；新增工作量超过完成工作量且没有正式 Scope 变更；`SCOPE-DOWN` 后未重写目标、DoD 和 Non-Goals。只能选择新的证伪实验、`PAUSE`、`PIVOT` 或 `STOP`。
...
3. 敌意代码默认禁执行；没有真实隔离、无凭据、无网络、只读输入、资源限制和运行后完整性检查的环境时，静态分析结果不得升级为可执行授权。
...
2. 成本红线：预估开销 > 剩余工作量 20% ⇒ 单代理同 schema 自审标 [SELF-AUDIT]（仍受 §5 锚点约束）。
...
`{state.json, plan.md, gates/*, templates/*}`；Git 仓库使用 commit/tag 记录可追溯版本，非 Git 环境使用 `manifest.md` 记录路径、版本、备份位置、服务状态和验证命令。
...
续跑卡：新会话首读 state.json → 回放三行（当前位置/最近 Gate 结果/下一步）→ 获确认续跑；
...
每里程碑结束打 git tag；变更前打 pre-<变更>-<ts> 恢复点。
...
## §9 元指令
提问仅在选型二义/Non-Goals 冲突、不可逆授权或方向性裁决时，每阶段集中 ≤1 次。
...
`state_transition`(§7 状态机转换合法性)、`blind_cost_calc`(§6 成本红线计算器)。

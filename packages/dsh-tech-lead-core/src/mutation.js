...
const scanMarkers = (root) => {
  ...
  while (stack.length) {
    ...
    nodes += 1;
    if (!stoppedAt && depth > SCAN_BUDGET.depth) { stoppedAt = { kind: 'depth', path: base || '/', limit: SCAN_BUDGET.depth }; continue; }
    if (!stoppedAt && nodes > SCAN_BUDGET.nodes) { stoppedAt = { kind: 'nodes', path: base || '/', limit: SCAN_BUDGET.nodes }; continue; }
...
export function previewMutation(raw) {
  ...
  for (const field of ['target', 'expectedDiff', 'verification', 'recoveryPoint', 'authorization']) {
    const part = scanMarkers(raw[field]);

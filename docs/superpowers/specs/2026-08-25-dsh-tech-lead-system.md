# Tech Lead 系统化（Skill v5.4 + DSH 插件）设计规格
...
| Target | 缺陷 3/3 关闭；核心校验器与插件单测全绿；真实 Cordis 组合 9/9 工具 PASS；headless+web profile 加载成功 |
...
└── packages/
    ...
    ├── dsh-tech-lead-plugin/    Cordis 插件：ctx.tools.register 注册 9 个只读工具
...
接入方式：`dsh plugin --profile <name> add <本地路径或npm>`（CLI 自动写 profile package.json 的
...
2. **Phase B/C**：core 每函数 ≥3 用例（正常/违例/边界）node:test；plugin 组合测试经真实
   Loader（vendor cordis bin.js）驱动 9 工具各一次，断言关键字段，打印 `TLT-PASS n/9`。

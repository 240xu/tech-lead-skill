...
**Steps:**
- [ ] Step1 写失败测试 `tests/installer.test.js`（node:test）：临时目录安装→断言文件全集含 state.json 与 marker.files 精确匹配；重装产生 .bak；写入外来文件后 uninstall→外来文件仍在且目标仅剩空骨架/被清空的受管集；--check 在删掉一个受管文件后 exit 1；--dry-run 后目录无变化。
- [ ] Step2 `node --test tests/installer.test.js` → 预期 FAIL（旧实现缺 state.json、递归删除）。
- [ ] Step3 重写 install.js 实现上述契约（walk 函数收集相对路径；copyWithBackup 复用；uninstall 逐文件 unlink）。
- [ ] Step4 测试全绿；`node tests/installer.test.js` exit 0。
- [ ] Step5 commit `fix(installer): managed-file manifest, safe uninstall, check/dry-run (v5.4)`
...
- [ ] Step1 state.test.js：合法 fixture 过；mode/tier/last_outcome 越界报错；done 缺 anchor 报错；evidence 缺 repro 报错且 level=E9 越界报错；unknown 字段→warning+记录不判 invalid；schema_version "1" 字符串兼容通过。
- [ ] Step2 红→绿循环实现 state.js。
- [ ] Step3 更新模板与 SKILL.md 条款；`git diff --check` 干净。
- [ ] Step4 commit `feat(core): machine-checkable state schema v1 validator`
...
**Steps:**
- [ ] Step1 每模块先写失败测试（≥3 例：正常/违例/边界）再实现，红绿循环。
- [ ] Step2 `cd packages/dsh-tech-lead-core && node --test tests/` 全绿。
- [ ] Step3 根回归：`node --test tests/`（A1+A2）仍绿。
- [ ] Step4 commit `feat(core): pure validators for classify/evidence/plan/gate/release/install/resume`
...
**Steps:**
- [ ] Step1 探测 registry：`npm view @deepseek-ai/dsh-tools version` → 决定 dep 形态。
- [ ] Step2 实现 index.js：9 个 defineTool 包装 core；入参 schema 与 render 全部落码。
- [ ] Step3 组合测试：cordis.yml = system-prompt + tools + plugin + driver.ts；driver 经
...
- [ ] Step4 commit `feat(plugin): register nine read-only tech-lead tools`
...
**Steps:**
- [ ] Step1 bundle 包：patch 内容见规格 §4.3；package.json 带 dsh.bundle.patch + plugin 依赖。
- [ ] Step2 `dsh plugin --profile techtest add <abs path to repo>/packages/dsh-tech-lead-bundle`
      （CLI 自动 init profile+bundles+pnpm）。
- [ ] Step3 加载验证：headless 一次性任务或最小 cordis 启动确认 tech-lead 工具注册日志出现；
      失败则回滚 profile（还原 .bak）并修 patch 形状。
- [ ] Step4 commit `feat(bundle): opt-in dsh bundle exposing read-only tech-lead tools`

### Task D2: headless → web 实装（每步独立复核）

- [ ] Step1 备份两 profile 的 package.json+cordis.patch.yml → `.bak-techlead-<ts>`。
- [ ] Step2 headless add bundle → 启动加载验证 → 通过才继续。
- [ ] Step3 web add bundle → 启动加载验证。
- [ ] Step4 任一步失败：还原该 profile 备份，问题修复后重走该步。
- [ ] Step5 commit（如有 repo 内产物变更）+ 记录安装结果到本计划勾选框。

### Task E1: 收尾

- [ ] Step1 版本：root 5.4.0；三个新包各自 0.1.0；README 双语补「DSH 插件」章节。
- [ ] Step2 全量回归：installer+state+core+composition 四套全绿。
- [ ] Step3 发布审计：releaseAudit 白名单扫描（本地路径/token 正则）；`npm pack --dry-run` 清单核对。
